...
if [ $ARCH = "64" ]; then
  # Get build version
  MAJOR=$(cat $DIR/vendor/jedi/vendor.mk | grep 'ROM_VERSION_MAJOR := *' | sed  's/ROM_VERSION_MAJOR := //g')
  MINOR=$(cat $DIR/vendor/jedi/vendor.mk | grep 'ROM_VERSION_MINOR := *' | sed  's/ROM_VERSION_MINOR := //g')
  MAINTENANCE=$(cat $DIR/vendor/jedi/vendor.mk | grep 'ROM_VERSION_MAINTENANCE := *' | sed  's/ROM_VERSION_MAINTENANCE := //g')
  TAG=$(cat $DIR/vendor/jedi/vendor.mk | grep 'ROM_VERSION_TAG := *' | sed  's/ROM_VERSION_TAG := //g')
